...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,dropout = None,joint=False, emb_dropout = 0.,T=None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        # pdb.set_trace()
        x = self.ln_post(x[:, 0, :])  # x: [bs, 197, 768]

        if self.proj is not None:  # proj: [768, 512]
            x = x @ self.proj  # [bs, 512]
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if tsm:
            print('=========using TSM==========')
            from temporal_shift import make_temporal_shift_vit
